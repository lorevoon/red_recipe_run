...
public class PlayerController : Singleton<PlayerController>
{
    ...
    private float _maxSpeed = 3f; // This will be updated from UpgradeManager
    private float _baseMaxSpeed = 3f; // Store the base speed
    ...
    {
        if (Input.GetKeyDown(KeyCode.Z))  // Use Z key to pick up ingredients
        {
            // Try to pick up an ingredient
            _playerInventory.TryPickUpIngredient();
        }

        if (Input.GetKeyDown(KeyCode.X))  // Use X key to drop ingredients
        {
